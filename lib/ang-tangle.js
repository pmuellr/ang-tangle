...
main = function(iDir, oFile, options) {
  var content, file, fileName, fileNames, files, initScript, name, oFileMap, out, script, scriptPrefix, scriptSuffix, scripts, _i, _len;
...
processViews = function(files) {
  ...
  file.contents = "AngTangle.module.constant \"views\", " + (JSON.stringify(views, null, 4));
  return files[file.name] = file;
};

processData = function(files) {
  var data, dirBaseName, err, file, json, name, object;
  data = {};
  for (name in files) {
    file = files[name];
    if (file.kind !== "data") {
  ...
  file.contents = "AngTangle.module.constant \"data\", " + (JSON.stringify(data, null, 4));
  return files[file.name] = file;
};
